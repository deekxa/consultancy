      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          <aside className="lg:w-1/3 bg-white/90 rounded-2xl border border-[#e5e0ff] p-6 shadow-sm backdrop-blur-sm h-max lg:sticky lg:top-24">
            ...
            <Link href="/services">
              <button className="mt-6 w-full rounded-xl bg-[#110053] text-white text-sm font-semibold py-2.5  hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
                Talk to a counselor
              </button>
            </Link>
          </aside>
          ...
          <div className="lg:w-2/3 grid gap-6 md:grid-cols-2">
            {tileServices.map((item, index) => (
              ...
              >
                ...
                <Link href={item.link ? item.link : `/services/${item.key}` }>
  <button
    className="inline-flex w-max items-center justify-center rounded-full border border-[#d4d4dd] bg-white px-5 py-2 text-xs sm:text-sm font-medium text-[#110053] hover:border-[#a89fff] hover:bg-[#f9f7ff] transition-colors"
  >
    {item.cta}
  </button>
</Link>
